 ...
 * XXX more blurb about what op_pv contains - i.e bundles of actions
 * interspersed with args
 */

PP(pp_multideref)
{
    SV *sv = NULL; /* init to avoid spurious 'may be used uninitialized' */
    MDEREF_item *items = (MDEREF_item*)(cPVOPx(PL_op)->op_pv);
    UV actions = items->actions;

    assert(actions);
    /* this tells find_uninit_var() where we're up to */
    PL_multideref_count = 0;

    while (1) {
        /* XXX explain that this is basically 2 chunks of code,
         * the AV and HV variants, each with various entry points and
         * 'do something first; goto entry point' variants,
         * and the end of each chunk returns on MDEREF_FLAG_last
         */
        ...
        case MDEREF_AV_rv2av_aelem:                  /* ->[...] */
            ...
            /* XXX can it ever be a SVt_PVAV/SVt_PVHV? */
